...
- **`users`**: Bảng trung tâm. Cột quan trọng: `role` (user/admin), `medical_conditions` (Mảng bệnh lý). Soft delete áp dụng qua cột `deleted_at`.
  - **Auth Cols (file 10):** `token_version`, `failed_login_attempts`, `locked_until` — hỗ trợ session invalidation & brute-force protection.
  - **Verification Code Cols (file 14):** `verification_code`, `verification_code_expires_at` — Mã PIN 6 số xác thực email. `reset_code`, `reset_code_expires_at` — Mã PIN 6 số đặt lại mật khẩu. *(Thay thế JWT token dài trong luồng Auth mới.)*
- **`user_relationships`**: Mapping (Many-to-Many) giữa Users. Ràng buộc cứng: User B chỉ được xem data của User A nếu có mapping tại đây. Có cột `is_primary` ưu tiên liên lạc.
- **`emergency_contacts`**: Lưu số điện thoại (từ ngoài hệ thống) để gọi khẩn cấp dựa trên `priority` (1-5).
- **`password_reset_tokens`** *(file 11 - DEPRECATED)*: Bảng phụ lưu JWT reset token cũ. Không còn dùng trong Auth Service mới (thay bằng cột `reset_code` trong bảng `users`).
